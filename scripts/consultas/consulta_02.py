# -*- coding: utf-8 -*-
"""
CONSULTA DT01 – KPI de Disponibilidad GPS por Dispositivo (últimas dia nayural de ayer)
...
-- Ventanas: día natural de ayer
limites AS (
  SELECT
    DATE_TRUNC('day', NOW() - INTERVAL '1 day') AS inicio_dia,
    DATE_TRUNC('day', NOW())                    AS fin_dia
),
...
gps_stats_24h_all AS (
  ...
  LEFT JOIN "DeviceLocations" dl
    ON dl."DeviceId" = d."Id"
   AND dl."Time" >= l.inicio_dia
   AND dl."Time" <  l.fin_dia
  GROUP BY d."Id"
),
